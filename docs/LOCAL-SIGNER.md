...
The availability command must report `secureEnclaveAvailable: true` before provisioning. `provision` creates one new local key and stores the opaque key reference in macOS Keychain. It prints `qx` and `qy`; copy those public coordinates into the [owner portal](PORTAL.md), call `AgentAccountFactory.createAgentP256(qx, qy, salt)` from the human owner's wallet, or use `agentic_create_identity` to prepare that owner transaction. The MCP tool never sends it. Do not provision again with the same label; the helper will fail rather than replace an existing key. The account's onchain `authenticatorP256()` must match this local key. Set `signer.kind`, the absolute `binaryPath`, and `label` in `.agentic-world.json` as shown in [`mcp/config.example.json`](../mcp/config.example.json). Set only `AGENTIC_WORLD_CONFIG` in the MCP host environment; do **not** set `AGENTIC_WORLD_OPERATING_KEY` for this mode.
