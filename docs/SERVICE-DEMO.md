...
From the repository root, run `npm run demo` and leave that terminal open. This single command builds contracts, SDK, MCP, pages, and signer; starts a fresh Osaka Hardhat node; deploys `RealEntryPoint` and `AgentAccountFactory`; and serves the owner portal and Service A. It prints their **actual** loopback URLs, the chain RPC, a random Service A operator key, and a Codex MCP registration command. If default ports are occupied, it chooses free nearby ports instead. Point your browser wallet's local network to the **printed RPC URL** before using the portal. The portal receives the fresh deployment at `/demo-deployment.json` and checks its deployment-block fingerprint against your wallet RPC, so it refuses an older node with the same chain ID. No manual editing of `portal/config.ts` is needed.

The command also writes a gitignored `.agentic-world.demo.json` for the local MCP and `.agentic-world.demo-state.json` for the next Codex session to discover the actual URLs. Neither file contains the service operator key. The repo-local Agentic World skill is already present at `.agents/skills/agentic-world/SKILL.md`; start the new Codex session **from this repo** after using the printed `codex mcp add …` command. You do not need to download the skill.

`npm run demo` does **not** silently provision a persistent Secure Enclave key or submit an owner-wallet transaction. When ready, use the printed `agentic-signer provision <label>` command yourself, copy its public `qx`/`qy` into the printed owner portal URL, and create the P-256 agent with your human wallet. The launcher observes the factory's `AgentCreatedP256` event, matches its public key to your local signer, and writes that agent ID to both generated files. Start Codex **after** the terminal confirms detection so MCP loads it. Then paste the resulting `0xAGENT` into Service A, unlock the desk with the printed operator key, and select **Enroll agent**. Enrollment checks pinned clone code, a nonzero `owner()`, and an active authenticator scheme. It grants neither resource by default.

The page can stay open while you change permissions. Stopping the demo shuts down all three local servers. Restarting it creates a **new chain and deployment** and clears enrollments, challenges, sessions, permissions, and the activity log; all are intentionally in memory for this demo. The Secure Enclave key persists locally and may be reused; do not provision it again under the same label.
