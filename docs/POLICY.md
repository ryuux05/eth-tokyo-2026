...
> **v0 policy:** `AgentPolicyHook` uses the existing `PolicyEngine` rule format
> around the new ERC-7579 single-call execution path. The older EIP-7702 account
> remains only as a historical prototype. See [v0 implementation](V0-IMPLEMENTATION.md)
> for the account's supported paths and testing limits.
...
2. **Owner binding:** factory initialization stores the human transaction sender as immutable `owner()`; this does not establish or enforce a behavioral mandate.
3. **Execution policy:** the owner controls which onchain actions the operating authenticator may perform autonomously, which require an exact owner approval, and which are denied.
...
This is the constrained v0 implementation of the policy specification. The rules below describe the implemented format.
...
For a native-only rule, `token = address(0)` and `maxAmount = 0`; the purchase selector cannot be used as a native-only rule. For a token rule, `token` is the expected ERC-20 address, `maxValue = 0`, and `maxAmount` is in token base units. The only supported token-aware call in v0 is `purchaseCompute(address token,uint256 amount)` on the exact configured `target`. The engine requires precisely 68 calldata bytes, checks its selector, decodes those two ABI arguments in their fixed positions, and compares both the token and amount. It never tries to infer token amounts from arbitrary calldata. During execution the account also compares its ERC-20 balance before and after the call and reverts if the target spent more of that token than the declared amount. A target contract must still implement trustworthy purchase behavior; the policy does not audit what it does with the payment or other assets.

The contract rejects unknown versions, malformed or noncanonical ABI encoding, zero targets, more than 32 rules, and policies larger than 8192 bytes. The hook stores the canonical bytes and the account exposes `policy()`, `policyHash()`, `policyRevision()`, `evaluateAction(...)`, and `ownerApprovalNonce()`. Updating the policy increments its revision but does not change the owner, authenticator, or service-side entitlements.

The agent SDK exports `Decision`, `TOKEN_PURCHASE_SELECTOR`, `encodePolicy(rules)`, `decodePolicy(bytes)`, and `ownerActionTypedData(action)`. The [owner portal](PORTAL.md) uses these to build and read policy:
...
The operating authenticator signs an ERC-4337 UserOperation whose `callData` invokes `execute(bytes32,bytes)` or `executeWithApproval(...)`. The configured EntryPoint is the only caller that can execute it. The account supports only ERC-7579 single-call, revert-on-error mode; the hook checks each call before and after execution. Anyone may fund the account through its payable `receive()` function. The operating signer cannot call `setPolicy` or owner-only lifecycle functions.
...
The approval nonce is separate from service proof nonces and the ERC-4337 UserOperation nonce. It advances before the external target call. A failed transaction rolls the increment back. Reentrancy is blocked during execution. The contract checks policy **before** any approval, so even a valid owner signature cannot bypass `DENY`.
...
This policy constrains the **account's supported onchain execution path**, not the agent's offchain/model behavior or a service's resource permissions. V0 has no agent-root EOA: the factory deploys an immutable-implementation clone owned by the human transaction sender. Services pin that implementation and independently decide access.

Only native `value` and the declared and net-spent amount of the exact token in the supported `purchaseCompute(address,uint256)` call are bounded in v0. Gas, ERC-20 `approve` amounts, arbitrary token calls, other token balances, cumulative spend, and downstream target behavior are not analyzed. The contract performs a normal `call`, not a `delegatecall`, but an approved target may itself call others. Narrowly pin targets and selectors. This is a hackathon prototype, not an audit or a universal spending firewall.

The legacy Hardhat EIP-7702 tests cover the original policy engine and approvals. Separate v0 Hardhat tests cover the new account's fixed modules, EntryPoint caller boundary, default deny, owner approval/replay, signer revocation, and the 2/20 token rule with overcharge rejection. A local test also executes an allowed UserOperation through the official EntryPoint v0.8 contract. It does **not** exercise a bundler or constitute an audit.
