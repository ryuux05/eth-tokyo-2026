...
`npm run build:sdk` emits JavaScript and declarations under `dist/sdk`. This remains a private repository package; public package versioning is not implemented. `sdk/deployments.ts` pins the Sepolia deployment across consumers; read the [deployment caveat](V0-IMPLEMENTATION.md) for source changes awaiting redeployment. Historical `agentInitializationTypedData`, `agentRegistrationTypedData`, `mandateRegistryAbi`, and `isExpectedDelegation` exports remain for the EIP-7702 prototype only; they are not needed for v0 account creation.
