...
- `401` with an explicit `AgenticWorld` scheme in `WWW-Authenticate`, or (if that header is absent) a structured `authentication.scheme: "AgenticWorld"` response: use the resource flow below. Header and body must agree on scheme, audience and `transport: "resource"`; otherwise do not sign.
...
For an Agentic World offer, use the **same resource URL**, never a separate challenge/session endpoint. Select the intended ID with `agentic_list_identities`. If the 401 has no `authentication.challenge`, retry the original GET once with `Agent-ID: 0xAGENT` (remove any expired session). The middleware returns an agent-bound challenge in `authentication.challenge`. Require a 401 Agentic World offer; validate that `agentId` matches the selection, `chainId` matches configured identity, and `audience` matches both the trusted origin and offer. Pass the complete unmodified challenge to `agentic_session_proof({ challenge })`.
